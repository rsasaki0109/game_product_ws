...
import { randomNormal, randomMonster } from '../data/customers.ts'
import { getNextUid } from './ids.ts'

export function spawnCustomer(elapsed: number, occupiedLanes: number[], forceMonster = false): CustomerInstance | null {
  // Monster chance: 0% for first 15s, then 10% ramping to 50%
  let monsterChance = 0
  if (elapsed > 15) {
    monsterChance = Math.min(0.5, 0.10 + (elapsed - 15) * 0.003)
  }
  const isMonster = forceMonster || Math.random() < monsterChance
  const def: CustomerDef = isMonster ? randomMonster() : randomNormal()
